...
st.title("🚀 AI-Powered Crypto Analyzer (DEBUG FIXED MODE)")
...
def load_data(symbol):
    ...
    if df.empty or not all(col in df.columns for col in ['Open', 'High', 'Low', 'Close']):
        st.error("⚠️ Missing one or more required OHLC columns.")
        return None
    ...
    try:
        ...
        st.write("Close shape:", close.shape)
    ...
    st.write("Data sample:", df[['Close', 'SMA_50', 'SMA_200', 'RSI', 'MACD']].tail())
    return df
