...
def plot_candlestick(df):
    ...
    fig.update_layout(title=f"📈 Candlestick Chart with Signals", xaxis_title='Date', yaxis_title='Price ($)',
...
if ticker:
    ...
    if df is not None:
        ...
        signal = df['Signal'].iloc[-1] if 'Signal' in df and not df['Signal'].empty else 0
        if signal == 1:
